...
```bash
# .git이 파일이면 worktree, 디렉터리면 main repo
if [ -f .git ]; then
  IS_WORKTREE="true"
  echo "✅ Worktree에서 실행 중: $(pwd)"
else
  IS_WORKTREE="false"
  echo "📍 Main repo에서 실행 중: $(pwd)"
fi
```

**2. 브랜치명 추출 (Main repo인 경우)**

- JIRA ID 추출: 사용자 입력에서 `JIRA-123` 또는 `PROJ-456` 형식 파싱
- 브랜치명 생성: `feature/JIRA-123`
- JIRA ID 없으면 사용자에게 입력 요청

```bash
# JIRA ID 파싱 (사용자 입력 또는 JIRA URL에서)
...
**3. Worktree 생성 및 이동 (Main repo인 경우)**
...
if [ "$IS_WORKTREE" = "false" ]; then
  ...
  if git worktree add "$WORKTREE_PATH" -b "$BRANCH_NAME" 2>/dev/null; then
    echo "✅ Worktree 생성 완료: $WORKTREE_PATH"
    ...
    WORKTREE_PATH="../worktrees/$NEW_BRANCH_NAME"
    git worktree add "$WORKTREE_PATH" -b "$NEW_BRANCH_NAME"
    cd "$WORKTREE_PATH" || exit 1
    echo "✅ Worktree 생성 완료: $WORKTREE_PATH"
  fi
else
  echo "ℹ️ 이미 worktree에서 실행 중입니다. 계속 진행합니다."
fi
```
...
- 기존 Phase 1-6 실행
- Phase 6에서 REPORT 파일 저장 시 worktree에 저장됨 (자동)

**5. Git 커밋 (Phase 6: Report Generation 완료 직후)**

Phase 6에서 REPORT 파일 생성 후 즉시 Git 커밋:

```bash
# REPORT 파일 커밋 (Phase 6 끝)
REPORT_FILE="[ISSUE_ID]_REPORT.md"

if [ -f "$REPORT_FILE" ]; then
  git add "$REPORT_FILE"
  git commit -m "docs: add analysis report for [ISSUE_ID]

Generated by analyze-issue skill"
  echo "✅ REPORT 커밋 완료: $REPORT_FILE"
else
  echo "⚠️ REPORT 파일을 찾을 수 없습니다: $REPORT_FILE"
fi
```
