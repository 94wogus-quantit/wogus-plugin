...
if [ -f .git ]; then
  ...
  echo "ℹ️ Main repo에서 실행 중입니다"
  echo "Worktree 사용 시 작업별 격리가 가능합니다"
fi
```

**Note**: document skill은 main repo와 worktree 모두에서 실행 가능합니다. Worktree에서 실행 시 Phase 9에서 worktree 정리 옵션이 제공됩니다.

**2. Phase 1로 진행**

- 기존 Phase 1-8 실행
- README.md, CHANGELOG.md 등 문서 업데이트는 worktree에서 수정됨 (자동)
...
#### 9D. Git Commit and Push (Worktree only)

**If running in a worktree** (`.git` is a file):
...
if [ -f .git ]; then
  ...
  if [ -n "$(git status --porcelain)" ]; then
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
      git add .
      git commit -m "docs: update documentation and code for $(git branch --show-current)

Generated by document skill"
      echo "✅ 변경사항 커밋 완료"
    else
      echo "❌ 커밋되지 않은 변경사항이 남아있습니다"
      echo "Worktree 삭제 전 커밋을 권장합니다"
      exit 1
    fi
  fi
...
**When to run**: After all documentation updates are complete (Step 1-4 완료 후).

#### 9E. Worktree Cleanup Options
...
if [ -f .git ]; then
  ...
  echo ""
  echo "[Y] 예 - Worktree 삭제 및 main repo로 복귀"
  echo "[N] 아니오 - Worktree 유지 (나중에 수동 삭제)"
  echo "[A] 아카이브 - Worktree를 아카이브 폴더로 이동"
  echo ""
  read -p "선택하세요 [y/N/a]: " -n 1 -r
  echo
  ...
  case $REPLY in
    ...
    [Aa])
      # 아카이브
      ARCHIVE_DIR="../worktrees-archive/$(date +%Y%m%d)-$(git branch --show-current)"
      mkdir -p ../worktrees-archive
      mv "$CURRENT_WORKTREE" "$ARCHIVE_DIR"
      cd "$MAIN_REPO"
      echo "✅ Worktree 아카이브: $ARCHIVE_DIR"
      echo "📍 현재 위치: $(pwd)"
      ;;
    *)
      # 유지
      echo "ℹ️ Worktree 유지됨: $CURRENT_WORKTREE"
      echo "수동 삭제: git worktree remove $CURRENT_WORKTREE"
      ;;
  esac
fi
```
...
**Risk Mitigation**:
- **Uncommitted changes**: Automatically detected in Step 9D, prompts user to commit
- **Unpushed commits**: Shows count and offers immediate push
- **Archive option**: Preserves worktree without deleting (useful for reference)
