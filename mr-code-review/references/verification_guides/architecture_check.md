...
1. **Serena로 프로젝트 아키텍처 이해**
   - `mcp__plugin_workflow-skills_serena__list_memories()`로 저장된 아키텍처 정보 확인
   - `mcp__plugin_workflow-skills_serena__read_memory()`로 아키텍처 결정사항 확인
...
3. **Serena로 기존 패턴과 비교**
   - `mcp__plugin_workflow-skills_serena__search_for_pattern()`으로 유사 코드 패턴 찾기
