...
- 기존 Phase 1-3 실행
- Phase 2 (Finalization)에서 PLAN 파일 저장 시 worktree에 저장됨 (자동)

**3. Git 커밋 (Phase 3: Finalization 끝 - PLAN 파일 저장 후)**

Phase 3에서 PLAN 파일 생성 및 승인 완료 후 즉시 Git 커밋:

```bash
# PLAN 파일 커밋 (Phase 3 끝)
PLAN_FILE="[FEATURE]_PLAN.md"

if [ -f "$PLAN_FILE" ]; then
  git add "$PLAN_FILE"
  git commit -m "docs: add implementation plan for [FEATURE]

Generated by plan-builder skill"
  echo "✅ PLAN 커밋 완료: $PLAN_FILE"
else
  echo "⚠️ PLAN 파일을 찾을 수 없습니다: $PLAN_FILE"
fi
```
